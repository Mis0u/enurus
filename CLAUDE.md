...
Chantiers à discuter/planifier avant l'annonce publique. Pas d'ordre de priorité figé — #24 est
le premier discuté, pas nécessairement bloquant.

| # | Item |
|---|------|
| 24 | **Page d'historique par exercice** — maquette validée : https://claude.ai/code/artifact/7f1ed236-07c5-4280-92f9-6478c10a9147. Accès depuis une carte exercice de la Bibliothèque. Architecture et décisions produit actées ci-dessous, **implémentation pas commencée**. **Architecture** : `ExerciseVoter::VIEW` (nouveau — seuls CREATE/EDIT/DELETE/RESTORE existent aujourd'hui ; `true` si `exercise.isPublic \|\| exercise.owner === user`) ; `ExerciseSetRepository::findSessionHistoryForExerciseAndUser(User, Exercise): array` — une ligne par séance agrégée (MAX poids/durée/distance, SUM reps, COUNT sets), triée chronologiquement ASC, via la base `queryForUser()` existante (garantit `w.owner = :user`, jamais les séances d'un autre utilisateur sur un exercice public partagé) ; pas de filtre SQL par période, le dataset (un seul exercice/user) est déjà petit, filtrage en PHP ; `ExerciseHistoryDataService` — détection PR via une nouvelle boucle "running max" locale scoped à un exercice (ne pas toucher `WorkoutRecordDetectionService::detectNewRecordEvents`, qui jette la valeur du record, nécessaire ici pour le graphique), calcule aussi les stats, pagine le tableau PHP déjà construit via `KnpPaginator::paginate($array, ...)` plutôt qu'une requête SQL paginée séparée ; `ExerciseHistoryChartBuilder` — `symfony/ux-chartjs` (déjà utilisé par le dashboard, jamais de Chart.js JS brut), courbe miroir de `DashboardTonnageChartBuilder`/`DashboardTonnageService`, rendu via `{{ render_chart(...) }}`, zéro JS custom pour le graphique ; `ExerciseHistoryController` Single Action, route 8 locales `/bibliotheque/exercice/{id}/historique`, `#[IsGranted('ROLE_USER')]` classe + `denyAccessUnlessGranted(ExerciseVoter::VIEW, $exercise)` en 1ère ligne ; pagination du journal via `_pagination.html.twig` déjà partagé ; traductions domaine `navigation`, namespace `exercise.history.*`. **Décisions produit** : bloc **Aperçu** (6 tuiles, toujours all-time, jamais affecté par le filtre de période) — Record actuel (🏆, même emoji que le reste du site) + date, Séances totales, Volume total (kg / m / min selon `measurementType`), Dernière fois pratiqué, Progression % depuis la 1ère séance, Fréquence moyenne ; bloc **Progression** (courbe + journal, seul affecté par le filtre) — pills "Tout" / "3 mois" / "1 an" **glissant** depuis aujourd'hui (jamais calendaire, contrairement aux filtres semaine/mois de Mes séances), chaque pill n'apparaît que si l'historique de CET exercice remonte effectivement jusque-là, défaut = "Tout" ; courbe en ligne, points PR visuellement distincts (pleins vs. creux), record actuel avec halo, règle stricte "supérieur au précédent" (égalité jamais un nouveau PR, même convention que partout ailleurs) ; journal des séances (Date / Série max / Volume / badge PR) — **Série max = la série à la valeur record de la séance (poids/durée/distance max), avec ses propres reps**, jamais une moyenne ni une fusion entre séries (ex. 80kg×10 + 90kg×8 + 100kg×5 → affiche "100 kg × 5"), Volume = tonnage cumulé de toutes les séries de la séance sur cet exercice. **Hors scope V1** : répartition reps/poids (pas convaincant), export/partage (prochaine version). |
