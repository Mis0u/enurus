...
| 27 | **Exercices mesurés par durée** (lié à #24, besoin réel confirmé — ex. gainage 8 min) — **fait pour `TIME`** : `MeasurementType` (`WEIGHT_REPS` par défaut / `TIME`) sur `Exercise`, `ExerciseSet::duration` (nullable, secondes), switch verrouillé à la création/édition d'exercice dès qu'une série existe (`ExerciseSetRepository::existsForExercise`). PR = durée max seule (poids ajouté ignoré du calcul, même convention que les reps pour le PR de poids), fusionné dans le même flux d'événements que le PR de poids (`WorkoutRecordDetectionService::findPrEvents`). Tonnage : poids seul si renseigné (comme une série à 1 rep), jamais poids × durée — unités non comparables. **Reste à faire** : `DISTANCE` (ex. farmer walk) — même direction (`ExerciseSet::distance` nullable, mètres), pas encore commencé. |
